        ...
        <div className='px-10 w-3/4  h-[650px] overflow-y-scroll felx flex-col'>
            ...
            <div className="relative flex flex-col w-full h-full text-gray-700 bg-white shadow-md rounded-xl bg-clip-border">
                ...
                <div className="p-6 px-0 overflow-scroll">
                    {orders?.map((order, i) => {
                        return (
                            <div key={i} className="m-10 flex flex-col shadow-xl">
                                <div className="flex">
                                    {order?.gifts?.map((item, i) => {
                                        const filteredGifts = gifts.filter(gift => gift.gid === item);
                                        return (
                                            <div key={i} className="flex">
                                                <img className='h-24 w-24'  src={filteredGifts[i]?.image} alt="" />
                                                <p className='py-10 font-semibold mx-3'>{filteredGifts[i].name}</p>
                                            </div>
                                        )
                                    })}
                                </div>
                                <div className="px-10 flex">
                                    <img className='h-28 w-28 rounded m-4' src={order.customizedImage} alt="" />
                                    <div className="">
                                        <p className='p-5'>{order.customizedName}</p>
                                        <p className='p-5'>Gift Wrapper : {order.giftWrapColor}</p>
                                    </div>
                                    <div className="p-5">
