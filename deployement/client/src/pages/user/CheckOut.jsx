...
const CheckOut = () => {
  const { user, buyProduct, order, setOrder } = useStates();
      ...
      const options = {
        ...
        handler: async (res) => {
          setOrder({
            ...order,
            price,
            gifts: [buyProduct?.gid],
            uid: user?.user?.uid,
            payment: {
              payment_id: res.razorpay_payment_id,
              amountPaid: buyProduct?.price,
              status: "paid",
              paymentDate: new Date(),
              modeOfPayment: "Card"
            }
          });
          await UserService.Order({
            ...order,
            price,
            quantity:1,
            gifts: [buyProduct?.gid],
      ...
      <div className="w-full flex items-start py-20 mb-10 px-20 gap-4">
        <div className="px-10 w-8/12  h-[700px] my-10 flex flex-col overflow-y-scroll">
          ...
          <div className="flex justify-end mt-10 mx-40">
            <button className='w-7/12 flex justify-end' onClick={() => handlePayment(buyProduct?.price)}>
        ...
        <div className="flex flex-col w-4/12 my-10 text-darkGreen">
          ...
          <div className="flex flex-col mt-5 w-full">
            <div className="flex my-5 justify-between w-full">
              <img className='h-20 w-20' src={buyProduct?.image} alt="" />
              <div className="flex flex-col w-8/12 ">
                <p className='font-semibold line-clamp-2'>{buyProduct?.name}</p>
                <p className='font-semibold'><s className='font-normal'>₹{buyProduct.price + 100}</s> ₹{buyProduct.price}</p>
