...
const ProductDetails = ({ product }) => {
    ...
    const { order, setOrder } = useStates();
    function upload(e) {
        ...
        reader.onload = function (event) {
            ...
            fileUploadApi.post("", JSON.stringify(formData))
                .then(response => {
                    ...
                    const url = "https://lh3.googleusercontent.com/d/" + response.data.fileUrl.id;
                    setProfile(url);
                    setOrder({ ...order, customizedImage: url });
                })
                .catch(error => {
                ...
                <Collapse open={open}>
                    <div className="flex items-center my-5">
                        ...
                        <label className="block">
                            ...
                            <input type="file" onChange={upload} className="cursor-pointer block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-lightGreen file:text-darkGreen hover:file:bg-opacity-60" />
                        </label>
                    </div>
                    <div className="mb-5">
                        <p className='text-lg font-semibold'>Name</p>
                        <input onChange={(e) => setOrder({ ...order, customizedName: e.target.value })} type="text" className='outline-none w-[80%] mx-5 my-2 border border-darkGreen p-2 py-4 border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Name For Whom?...' />
                    </div>
                    <div className="">
                    ...
                    <div className="my-5">
                        ...
                        <input onChange={(e) => setOrder({ ...order, quote: e.target.value })} type="text" className='outline-none w-[80%] mx-5 my-2 border border-darkGreen p-2 py-4 border-opacity-30 focus:border-opacity-100 duration-200 rounded-sm' placeholder='Enter Quote...' />
                    </div>
                        ...
                        <div>
                            ...
                            <Collapse open={wrap}>
                                <div className="mx-14 my-5">
                                    ...
                                    <select onChange={(e) => { setOrder({ ...order, giftWrapColor: e.target.value }); console.log(order);}} size={1} name="" id="" className='p-2 outline outline-2 text-xl mx-5 mt-2'>
                                        {colors.map((data, i) => {
                                            return (
